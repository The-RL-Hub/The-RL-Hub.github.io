...
این مدل در واقع یه *میانگین متحرک با وزن‌دهی نمایی* حساب می‌شه که وزن داده‌های قبلی رو به صورت نمایی کمتر و کمتر می‌کنه.  
اگه این رابطه رو باز کنیم، بعد از $n$ بار آپدیت برای آرم $i$، می‌رسیم به این:

$$
\hat{Q}_{n}(i) = (1-\alpha)^{n} \hat{Q}_{0}(i) + \sum_{j=1}^{n} \alpha (1-\alpha)^{n-j} R_{(j)}^{(i)}
$$

که توش $R_{(j)}^{(i)}$ یعنی ریوارد $j$ام آرم $i$.  
